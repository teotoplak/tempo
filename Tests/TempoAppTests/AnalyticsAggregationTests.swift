...
final class AnalyticsAggregationTests: XCTestCase {
    ...
    func testDailySummaryBuildsProjectAndIdleBreakdownFromCheckIns() throws {
        ...
        XCTAssertEqual(summary.projectSummaries.map(\.projectName), ["Admin", "Idle", "Client Work"])
        XCTAssertEqual(summary.projectSummaries[0].totalDuration, 45 * 60, accuracy: 0.001)
        XCTAssertEqual(summary.projectSummaries[1].totalDuration, 20 * 60, accuracy: 0.001)
    ...
    func testMenuBarDayNavigationShowsPreviousDaySummary() throws {
        ...
        XCTAssertEqual(appModel.menuBarDayProjectSummaries[0].totalDuration, 30 * 60, accuracy: 0.001)
        XCTAssertEqual(appModel.menuBarDayProjectSummaries[0].percentageOfTotal, 1, accuracy: 0.001)
        XCTAssertEqual(appModel.menuBarDayWorkedDuration, 30 * 60, accuracy: 0.001)
